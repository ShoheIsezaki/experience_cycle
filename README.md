...
- **チェック（デイリーチェックリスト）**: 曜日ごとにオンにした「毎日のタスク」をこなす画面。日付ナビ付きで、その日にオンのタスクを時間順に表示し、丸をタップしてチェック / 解除します（過去日も閲覧・チェック可）。上部のグラデーション進捗カードに「done/total」のリング・状況文言・**連続全達成ストリーク🔥**（全タスク完了日が連続する日数。今日が未完了でも昨日までの連続を維持表示。2日以上で表示）を出します。下部の「チェックリストを編集」からタスクを追加・編集できます。
  - タスクは **名前（必須）・参考リンク（任意URL）・メモ（最大1000字）・目安の時間（HH:MM）・オンにする曜日（月〜日の複数選択、毎日 / 平日 / 週末プリセットあり）** を持ちます。
  - 「削除」はデータを消さずに**アーカイブ**（履歴保持）します。過去日の達成状況はアーカイブ後も当時のまま正しく再現されます。
- **カレンダー**: 月表示。各日に天気マークを表示し、その下に**達成数バッジ**「2/3」（全達成=緑 / 一部=黄 / 0=グレー。対象タスクが0の日は非表示）を出します。日をタップすると内容とチェック内訳（✓ / ○）を確認でき、そのまま編集画面へ。曜日ヘッダーの下に曜日テーマの帯を表示します（隣り合う曜日が同じテーマなら1つに結合表示）。
- **週ビュー**: 月曜始まりの週表示で7日分をリスト表示。各日ヘッダーに同じ達成数バッジを出します。
...
Supabase が設定済み（下記「クラウド同期」参照。`SUPABASE_SERVICE_ROLE_KEY` + `VITE_SUPABASE_URL`）の場合は、メッセージに「✅ 今日のチェック: 2/3」の行が追加されます（今日対象のタスクが1つ以上あるときのみ）。Supabase 未設定・取得失敗時は従来どおりのメッセージが送られます（通知自体は失敗しません）。
...
## Discord チェックリストのタスクアラート

チェックリストのタスクに「目安の時間」を設定しておくと、その時刻に Discord へ「🔔 「XXXX」の時間です！」というアラートを送る GitHub Actions（`.github/workflows/alerts.yml`）を用意しています。手動実行（workflow_dispatch）も可能です。

- **仕組み**: 10分間隔（cron `*/10 * * * *`）で実行され、JST の現在時刻から見て「直近10分の窓（now-10分 〜 now）」に「目安の時間」が入っていて、その日の曜日がオンのタスクを Supabase から取得し、1タスクにつき1メッセージを送信します（メモ先頭200字・参考リンク・アプリへのリンクを含む）。
- **遅延について**: GitHub Actions のスケジュール実行には数分の遅延があり得ます。10分窓のポーリング方式のため、境界付近ではごく稀に通知の取りこぼしや重複が起こり得ます（設計上許容）。厳密な時刻厳守が必要な用途には向きません。
- **設定手順**: 「Discord リマインド通知」の `DISCORD_WEBHOOK_URL` と、「クラウド同期」の `SUPABASE_SERVICE_ROLE_KEY`（Secret）+ `VITE_SUPABASE_URL`（Variable）が必要です。どれか未設定の場合、このワークフローは失敗せずスキップされます（該当タスク0件のときも何も送らず正常終了）。
...
- テスト: Vitest（日付ユーティリティ・週計算・エクスポート/インポートのシリアライズ・チェックリストの対象判定/達成数/ストリーク・同期プラン）
...
5. **スキーマを作成**: Supabase の **SQL Editor** で、このリポジトリの [`supabase/schema.sql`](supabase/schema.sql) の内容を実行します（`entries` / `weekday_themes` / `checklist_items` / `checklist_checks` テーブルと RLS ポリシーが作成されます）。このファイルは**冪等（再実行可能）**なので、すでに一部のテーブルを作成済みでも、もう一度**丸ごと実行**するだけで不足しているテーブル（チェックリスト機能を追加した場合の `checklist_items` / `checklist_checks` など）が追加されます（エラーになりません）。既存データはそのまま保持されます。
...
- 競合は **updatedAt が新しい方を採用（Last-Write-Wins）** で解決します。削除は「空の記録（トンボストーン）」として同期され、端末間で伝播します。チェックリストのタスクはアーカイブ（`archived=true`）、チェックの解除（`checked=false`）がそれぞれトンボストーンとして伝播します。
- フル同期は `entries` → `weekday_themes` → `checklist_items` → `checklist_checks` の順に処理します。いずれかのテーブルが未作成（`supabase/schema.sql` 未実行）でも、そのカテゴリのみスキップし他の同期は継続します。
